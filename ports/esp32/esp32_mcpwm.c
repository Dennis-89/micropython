...
static mp_obj_t mcpwm_make_new(const mp_obj_type_t *type, size_t n_args, size_t n_kw, const mp_obj_t *args) {
    ...
    //char error_message = "bad timer id requested: %d (0~5 allowed)";
     if (block_id >= MCPWM_CHANNEL_MAX) {
        nlr_raise(mp_obj_new_exception_msg_varg(&mp_type_ValueError, MP_ERROR_TEXT("bad timer id requested: %d (0~5 allowed)"), block_id));
    }
...
//    { &mp_type_type },
//    .name = MP_QSTR_MCPWM,
//    .slot_index_print = mcpwm_print,
//    .slot_index_make_new = mcpwm_make_new,
//    .slot_index_locals_dict = (mp_obj_dict_t *)&mcpwm_locals_dict,
//};
